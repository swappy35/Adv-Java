...
public class ContextParamServlet3 extends HttpServlet{
	protected void doPost(HttpServletRequest resquest, HttpServletResponse response) throws ServletException, IOException{
		...
		out.println("<br>Addition of "+s1+" "+s2+" and "+s4+" : "+(i+j+l));
		int large;
		large = (i > j) ? (i > k ? i : k) : (j > k ? j : k);
		large = (large > l ? large : l);

		out.println("<br>Largest of "+s1+" , "+s2+" , "+s3+" & "+s4+" : "+(large));
